#!/usr/bin/env bash
# Structural smoke test for ruflo-todo-debt v0.1.0 (ADR-0001).
set -u
...
if [[ "$v" != "0.1.0" ]]; then
  ...
  miss=""
  for k in todo fixme tech-debt debt ci; do
    grep -q "\"$k\"" "$ROOT/.claude-plugin/plugin.json" || miss="$miss $k"
  done
  [[ -z "$miss" ]] && ok || bad "missing keywords:$miss"
fi

# 2. todo-debt-scan skill present with valid frontmatter
step "2. skills/todo-debt-scan/SKILL.md has name/description/allowed-tools"
F="$ROOT/skills/todo-debt-scan/SKILL.md"
miss=""
...
step "3. /todo-debt command covers all 5 subcommands"
F="$ROOT/commands/todo-debt.md"
miss=""
for sub in 'todo-debt scan' 'todo-debt check' 'todo-debt report' 'todo-debt badge' 'todo-debt gate'; do
  grep -q "$sub" "$F" || miss="$miss '${sub#todo-debt }'"
done
[[ -z "$miss" ]] && ok || bad "missing:$miss"

# 4. agent references REFERENCE.md (token-optimization pattern)
step "4. agent references REFERENCE.md"
grep -q "REFERENCE.md" "$ROOT/agents/todo-debt-analyst.md" \
  && ok || bad "REFERENCE.md cross-reference missing"

# 5. skill invokes scripts/scan.mjs
step "5. skill drives scripts/scan.mjs"
grep -q "scripts/scan\.mjs" "$ROOT/skills/todo-debt-scan/SKILL.md" \
...
ADR="$ROOT/docs/adrs/0001-todo-debt-gate.md"
